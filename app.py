...
def get_llm():
    ...
    if llm is None:
        ...
        llm = ChatGroq(api_key=api_key, model="llama-3.1-8b-instant", temperature=0, max_tokens=512)
    return llm

# STRONGER PROMPT - Forces LLM to use ONLY the context
PROMPT = ChatPromptTemplate.from_template(
    "You MUST answer ONLY using the document context below. "
...
def chat():
    ...
    try:
        ...
        docs = b.intelligent_search(q, k=4)
        ...
        return jsonify({"answer": f"Sorry, an error occurred. Please try again."})
